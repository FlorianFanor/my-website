...
const WorkItem = ({ data, index, isExpanded, onToggle }: WorkItemProps) => {
    ...
    return (
        <div className="relative w-full">
            ...
            <div className={`hidden md:flex items-start w-full ${isEven ? 'md:justify-start' : 'md:justify-end'}`}>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {stack.split(', ').map((tech, techIndex) => (
                                    <span key={`${tech}-${techIndex}`} className="text-xs bg-gradient-to-r from-slate-100 to-slate-200 text-slate-700 px-3 py-1 rounded-full font-medium hover:from-blue-100 hover:to-purple-100 transition-colors duration-300">
                                        {tech}
                                    </span>
                                ))}
                            </div>
                ...
                {isExpanded && (
                    <div className={`md:w-5/12 ${isEven ? 'md:order-2 md:ml-auto' : 'md:order-1 md:mr-auto'}`}>
